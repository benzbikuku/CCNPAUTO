...
domainname_pattern = re.compile(r'ip domain name (\S+)')
...
interface_prop_pattern = re.compile(r'interface (?P<int_name>\S+)(\n.+?)+\n?\s?ip address (?P<ip_address>[\d\.]+) (?P<mask>[\d\.]+)')
...
# print('\nInterface with IP'.ljust(18) + ':' + str(interface_list))
    ...
    # print(interface_properties_list, indent=10)
